...
function Register({ setIsLogin, setModalIsOpen }) {
        ...
        <>
            ...
            <div className='w-96 bg-white md:w-1/3 mx-auto shadow-md px-8 py-5 rounded-xl'>
                ...
                <form onSubmit={handleSubmit(data => handleFormSubmit(data))} className='flex flex-col gap-5'>
                    ...
                    >{!loading ? 'Register' : 'Registering.....'}</button>
                </form>
                <div className='text-center my-3'>
                    <span className='text-gray-700'>All ready have a account</span>
                    <div onClick={() => { setIsLogin(prev => !prev); setModalIsOpen(true) }} className='cursor-pointer text-blue-500 underline hover:text-blue-700'> Login</div>
                </div>
            </div>
        </>
    )
